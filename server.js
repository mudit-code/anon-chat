...
io.on("connection", (socket) => {
  ...
  socket.on("create-room", ({ roomKey, username }) => {
    ...
    rooms[roomKey] = {
      ...
      voiceUsers: new Set() // Track users currently in voice chat
    };
  ...
  socket.on("join-voice", ({ roomKey }) => {
    const room = rooms[roomKey];
    if (room) {
      // Send list of existing voice users to the new joiner
      const usersInVoice = Array.from(room.voiceUsers);
      socket.emit("voice-users", usersInVoice);

      // Add new user to voice list
      room.voiceUsers.add(socket.id);

      // Notify others that a new user joined voice
      socket.to(roomKey).emit("user-joined-voice", socket.id);
    }
  });

  socket.on("leave-voice", ({ roomKey }) => {
    const room = rooms[roomKey];
    if (room) {
